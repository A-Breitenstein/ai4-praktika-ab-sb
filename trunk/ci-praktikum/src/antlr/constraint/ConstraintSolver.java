...
//        }
    ...
    public void addOperationSpaltenweise(String op1, String op2, String result) {
        ...
        for (char c : op1.toCharArray()) {
            iv_op1_List.add(Choco.makeIntVar(String.valueOf(c),0,1,Options.V_BOUND));
        }
        ...
        IntegerVariable iv_startUebertrag = Choco.makeIntVar(String.valueOf("sUebertrag"), 0, uppB_op1, Options.V_BOUND);
        IntegerVariable iv_uebertrag = Choco.makeIntVar(String.valueOf("uebertrag"), 0, uppB_op2, Options.V_BOUND);

        ListIterator<IntegerVariable> op1IT = iv_op1_List.listIterator(iv_result_List.size());
        ListIterator<IntegerVariable> op2IT = iv_op2_List.listIterator(iv_result_List.size());
        ...
        for (int i = 0; i < iv_result_List.size() - 1; i++) {
            uebertraege.add(Choco.makeIntVar("uebertrag"+i, 0, 1));
        }
        ...
        while (resultIT.hasPrevious()) {
            ...
            if(op1IT.hasPrevious() && op2IT.hasPrevious())
            model.addConstraint(Choco.eq(Choco.plus(Choco.plus(op1IT.previous(), op2IT.previous()),uebertraege.get(indexOfUbertrag-1)), Choco.plus(resultIT.previous(), Choco.mult(uebertraege.get(indexOfUbertrag), 10))));
